...
-- ============================================
-- PRAGMA integrity_check returns a table with 'ok' or corruption details
-- Must SELECT from pragma_integrity_check (virtual table syntax)

SELECT 'DB_INTEGRITY',
    CASE
        WHEN UPPER(integrity_check) = 'OK' THEN 'PASS'
        ELSE 'FAIL'
    END AS result
FROM pragma_integrity_check LIMIT 1;
...
INSERT OR IGNORE INTO topics (id, topic_id, name, status) VALUES (999, 'orphan-test', 'Orphan', 'pending');
...
WHERE t.topic_id = 'orphan-test' AND f.topic_id IS NULL;
