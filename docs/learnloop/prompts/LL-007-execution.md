...
Parallelizable with: LL-005, LL-006, LL-008, LL-009
```
...
```bash
set -e
echo "=== PREFLIGHT CHECKS FOR LL-007 ==="
...
test -f docs/learnloop/mcp-queries/practice.sql || { echo "FAIL: practice.sql not found"; exit 1; }
...
```bash
cat > .superpowers/checkpoints/LL-007-START << 'EOF'
{"storyId": "LL-007", "createdAt": "'$(date -Iseconds)'", "gitRef": "'$(git rev-parse HEAD)'"}
EOF

jq '.LL-007 = {"status": "in-progress", "phase": 2, "title": "Practice Session Workflow", "startedAt": "'$(date -Iseconds)'", "dependencies": ["LL-004"]}' .superpowers/state/story-progress.json > tmp.json && mv tmp.json .superpowers/state/story-progress.json
```
...
```bash
git add SKILL.md docs/learnloop/mcp-queries/practice.sql
git commit -m "feat(LL-007): practice session workflow
...
jq '.LL-007.status = "passed" | .LL-007.completedAt = "'$(date -Iseconds)'"' \
  .superpowers/state/story-progress.json > tmp.json && mv tmp.json .superpowers/state/story-progress.json

cat > .superpowers/checkpoints/LL-007-PASS << 'EOF'
{"storyId": "LL-007", "status": "passed", "completedAt": "'$(date -Iseconds)'"}
EOF
...
#!/bin/bash
set -e
REF=$(jq -r '.gitRef' .superpowers/checkpoints/LL-007-START)
git checkout $REF -- SKILL.md docs/learnloop/mcp-queries/practice.sql
jq '.LL-007.status = "pending"' .superpowers/state/story-progress.json > tmp.json && mv tmp.json .superpowers/state/story-progress.json
rm -f .superpowers/checkpoints/LL-007-*
...
**END OF EXECUTION PROMPT FOR LL-007**
