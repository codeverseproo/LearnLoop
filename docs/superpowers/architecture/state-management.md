...
├── checkpoints/
│   ├── MIT-001-START              # Pre-execution snapshot
│   ├── MIT-001-PASS               # Success marker
│   └── MIT-001-rollback.sh        # Rollback script
└── worktrees/
    └── MIT-001/                   # Isolated worktree (if used)
```
...
```json
{
  ...
  "MIT-003": {
    ...
    "checkpoint": ".superpowers/checkpoints/MIT-003-START"
  }
}
```
...
```bash
#!/bin/bash
# .superpowers/checkpoints/MIT-001-rollback.sh
set -e
...
# Update state
jq '.MIT-001.status = "pending"' .superpowers/state/story-progress.json > tmp.json
...
```bash
#!/bin/bash
# Preflight checks for MIT-001

echo "Running preflight checks for MIT-001..."
...
# State file
jq -e '.MIT-001.status == "pending"' .superpowers/state/story-progress.json > /dev/null || { echo "FAIL: MIT-001 already processed"; exit 1; }
...
```markdown
## Manual Prerequisites for MIT-001
...
jq '.MIT-001.status = "in-progress" | .MIT-001.startedAt = "'$(date -Iseconds)'"' \
...
jq '.MIT-001.status = "failed"' .superpowers/state/story-progress.json > tmp.json && mv tmp.json .superpowers/state/story-progress.json
```
...
STATUS=$(jq -r '.MIT-001.status' .superpowers/state/story-progress.json)

if [ "$STATUS" = "in-progress" ]; then
  echo "Detected interrupted execution of MIT-001"
  STARTED=$(jq -r '.MIT-001.startedAt' .superpowers/state/story-progress.json)
...
# Usage
check_dependencies "MIT-003" || exit 1
```
